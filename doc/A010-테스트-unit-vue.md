...
아! 참고로 싱글 파일 컴포넌트 형식의 VUE 단위 테스트일 경우 입니다. 
...
초보 여러분은 이런 경우 단위 테스트를 작성한다고 생각하고 

통합 테스트 처럼 단위 테스트로 구현하려고 할 겁니다. 

거의 90% 로 ....

그래서 구성된 VUE 에 연결된 내용에 포함된 결과를 시험하고 싶고

사용자 입력을 시험하고 싶은 욕구에 휩싸이시면서 

여러분은 무의식 중에 E2E 테스트와 통합 테스트에서 해야 하는 것들을 단위 테스트에 구현하게 될 겁니다. 
...
이 단위 테스트 지원 유틸리티는 순수하게 단위 테스트만 할 수 있도록 여러가지 컴포넌트를 제공합니다. 

이 유틸리티는 이미 따라하기 과정에서 사용한 Vue CLI 3.0 에서 지원하고 있기 때문에 여러분은 

따로 설치할 필요는 없습니다. 그저 다음과 같은 형태로 포함하면 됩니다.
...
이것은 앱 어플리케이션 의 시작 소스인 main.js 에서 다음 처럼 Vue 인스턴스를 생성하고 

앱의 전체 컴포넌트들을 구성해 줍니다. 
...
위 루틴에서는 라우터와 스토어 공간을 생성하고 각 랜더링 대상을 처리 합니다. 

이것과 대신해 주고 가짜로 만들어 주는 것이 createLocalVue 입니다. 

그리고 테스트 하고자 하는 컴포넌트의 마운트를 흉내 내 주는게 shallowMount 입니다. 

이 두가지를 사용해서 마운트할 경우에는 적용대상 Vue 컴포넌트 자체만 랜더링하고 하부 컴포넌트 들은 렌더링 하지 않습니다. 
...
이렇게 단위 테스트 대상 싱글 파일 컴포넌트만 랜더링하고 처리 하는 것 

그리고 가짜 Vue 인스턴스를 생성하는 것

이게 단위 테스트에 도움이 되는 vue test util 모듈입니다. 

그외 다른 여러 가지 기능이 있습니다만 초보분들은 여기까지만 이해하시면 됩니다. 

아 한가지 모든 이벤트나 기타 등등의 처리가 비동기 처리되지 않고 동기 처리가 된다는 겁니다. 

즉 항상 호출 즉시 해당 루틴이 즉각 즉각 동작한다는 특성 역시 이해하셔야 합니다. 
...
이 함수는 자식 컴포넌트는 stub 로 대치된다고 앞에서 설명하였습니다. 
...
태그 이름은 원래 이름 뒤에 "-stub" 라는 접미사가 붙어서 만들어 집니다. 
...
여러분이 vue 를 이해하고 계신다면 v-mdel 은 사실 다음과 같은 형태의 다른 표현임을 알고 계실 겁니다. 
...
mock spy stub 의 개념은 설명히 너무 길어지니 그냥 stub 에만 신경 쓰시고 
이건 가짜 또는 대신 이라는 의미로 간단히 이해하시면 됩니다. 

이벤트가 발생하면 원래 동작해야 할 이벤트 처리 함수 대신에
sinon 을 이용하여 가짜 함수를 하나 만듭니다. 그리고 이 함수를 이용하여 
호출이 발생했는가를 검사 합니다. 
